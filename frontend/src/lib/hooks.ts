...
export const useReadERC20Balances = (
  tokenAddresses: `0x${string}`[],
  userAddress: `0x${string}`
): UseReadContractsReturnType => {
  ...
  return useReadContracts({
    ...
    query: {
      enabled: tokenAddresses.length > 0 && userAddress !== undefined,
    },
  })
}
...
export const useReadDepositManagerAllowances = (
  address: string,
  tokenIds: `0x${string}`[]
): UseReadContractsReturnType => {
